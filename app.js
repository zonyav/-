(() => {
    ...
    try {
      ...
      return {
        completed: Array.isArray(saved.completed)
          ? saved.completed.filter((id) => lessons.some((lesson) => lesson.id === id))
          : [],
      ...
      return { completed: [], theme: null, notes: {}, skills: [] };
    }
  }
  ...
  function renderNav(filter = "") {
    ...
    navEl.innerHTML = course.modules.map((module) => {
      const moduleCompleted = module.lessons.filter((lesson) => state.completed.includes(lesson.id)).length;
      ...
      return `<section class="nav-module">
        <p class="nav-module-title"><span>${module.number}</span>${module.title}<small>${moduleCompleted}/${module.lessons.length}</small></p>
  ...
  function isCorrectQuiz(quiz) {
    ...
    const text = quiz.querySelector('input[type="text"]');
    if (text) {
      const alternatives = (quiz.dataset.alternatives || "").split("|").filter(Boolean);
      return [quiz.dataset.answer, ...alternatives].map(normalize).includes(actual[0]);
    }
  ...
  function bindLearningTools() {
    lessonEl.querySelectorAll("textarea[data-note]").forEach((field) => {
      const key = `${current.id}:${field.dataset.note}`;
      field.value = state.notes[key] || "";
      field.addEventListener("input", () => {
        state.notes[key] = field.value;
        saveState();
        const status = field.closest(".workbook")?.querySelector("[data-note-status]");
        if (status) {
          status.textContent = "Сохранено на этом устройстве";
          clearTimeout(status.hideTimer);
          status.hideTimer = setTimeout(() => { status.textContent = ""; }, 1800);
        }
      });
    });

    const skillBoxes = [...lessonEl.querySelectorAll("input[data-skill]")];
    const updateSkillCounter = () => {
      const checked = skillBoxes.filter((box) => box.checked).length;
      lessonEl.querySelectorAll("[data-skill-progress]").forEach((label) => {
        label.textContent = `${checked} из ${skillBoxes.length} навыков отмечено`;
      });
    };
    skillBoxes.forEach((box) => {
      box.checked = state.skills.includes(box.dataset.skill);
      box.addEventListener("change", () => {
        state.skills = box.checked
          ? [...new Set([...state.skills, box.dataset.skill])]
          : state.skills.filter((skill) => skill !== box.dataset.skill);
        saveState();
        updateSkillCounter();
      });
    });
